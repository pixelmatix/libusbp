...
libusbp_error * libusbp_find_device_with_vid_pid_sn(
    uint16_t vendor_id, uint16_t product_id, const char * serial_number, libusbp_device ** device)
{
    if (device == NULL)
    {
        return error_create("Device output pointer is null.");
    ...
    *device = NULL;
    ...
    for(size_t i = 0; error == NULL && i < size; i++)
    {
        ...
        if (*device == NULL)
        {
            ...
            char * str;
            libusbp_device_get_serial_number(candidate, &str);
            libusbp_string_free(str);

            if (matches && str && !strcmp(str, serial_number))
            {
                // Return the device to the caller.
                *device = candidate;
                candidate = NULL;
